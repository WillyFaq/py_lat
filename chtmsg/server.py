    ...
    while True:
        try:
            ...
            if msg:
                ...
                regx = re.split("login ",msg_decode)
                if len(regx)>1:
                    broadcast(f'{regx[1]} join chat!', connection)
                    client[f'{address[0]}:{address[1]}'] = regx[1]
                    print(client)
                else:
                    # Build message format and broadcast to users connected on server
                    print(f'{address[0]}:{address[1]} - {msg_decode}')
                    c = client[f'{address[0]}:{address[1]}']
                    # print(f'{c} - {msg_decode}')
                    msg_to_send = f'{c} - {msg_decode}'
